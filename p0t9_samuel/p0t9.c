        ...
        for(int i = 1; i < *ROWS; i++){ // BEGIN THE BLUCLE TO DO THE CALCULATIONS
            for (int j = 1; j < *COLUMNS; j++){
             ...
             // COPY THE VALUE THAT IS IN THE POSITIONS
                int arribaIzq = (int) matrixOriginal[i-1][j-1];
                int arribaCen = (int) matrixOriginal[i-1][j];
                int arribaDer = (int) matrixOriginal[i-1][j+1];
                int centroIzq = (int) matrixOriginal[i][j-1];
                int centroCen = (int) matrixOriginal[i][j];
                int centroDer = (int) matrixOriginal[i][j+1];
                int abajoIzq  = (int) matrixOriginal[i+1][j-1];
                int abajoCen  = (int) matrixOriginal[i+1][j];
                int abajoDer  = (int) matrixOriginal[i+1][j+1];

                int V[9] = {arribaIzq, arribaCen, arribaDer, centroIzq, centroCen, centroDer, abajoIzq, abajoCen, abajoDer};
